<script lang="ts">
  export let onSubmit: (dataSource: { url: string; interval: number }) => void;

  export let dataSourceURL = "";
  export let intervalSec = 0.0;

  function handleSubmit(event: Event) {
    event.preventDefault();
    onSubmit({ url: dataSourceURL, interval: intervalSec * 1000 });
  }
</script>
          ...
          <div class="relative w-full max-w-md">
            ...
            <div class="mt-1">
              <input
                ...
                class="border border-gray-300 focus:outline-blue-400 rounded w-full h-10 p-3 pt-5 text-sm w-xs"
